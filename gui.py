...
class AutoGensis():
    def __init__(self,master):
        ...
        self.Configuration_frame = tk.Frame(self.main_frame)
        self.Configuration_frame.pack(pady=10,padx=10)

        self.Overview_frame = tk.Frame(self.main_frame)
        self.Overview_frame.pack(pady=10,padx=10)
        ...
        ########### configuration-Frame ##################
        self.Start_engine = tk.Button(self.Configuration_frame, text="Run Engine", command=self.run_engine)
        self.Start_engine.pack(pady=10,padx=10)
 
        self.add_video = tk.Button(self.Configuration_frame, text="Add Video to list", command=self.Add_Video_tolist)
        self.add_video.pack(pady=10, padx=10)

        self.stop_btn = tk.Button(self.Configuration_frame,text="Stop Engine",command=self.stop_engine)


        ############ Overview-Frame ####################
        self.Video_Amount = tk.Label(self.Overview_frame, text=f"VideoFiles added ({len(self.Uploaded_videos_tolist)})")
        ...
        Device_information.pack(pady=10,padx=10)
        

        ##### Agent ProgressLog - FRAME######
        self.agent_log_display = scrolledtext.ScrolledText(self.AgentUpdate,Wrap=tk.WORD, width=55,height=25,font=("Helvetica",12), bg="black",  fg="white",state="disabled",)
        self.agent_log_display.config(   insertbackground="yellow", selectbackground="#444444",selectforeground="white",borderwidth=2,relief="sunken")
