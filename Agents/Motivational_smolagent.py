...
def Motivational_analytic_agent(transcript_path,agent_txt_saving_path):
    ...
    global Global_model
    Global_model =  Reload_and_change_model("gpt-5-medium",message="Reloading model to -> gpt-5 before running [Motivational_analytic_agent]")
    ...
    while True:
        chunk = chunk_limiter.forward(file_path=transcript_path, max_chars=6250)
        if not chunk.strip():
