    ...
    def forward(self, inputs):
        ...
        try:
            ...
            if match_start != -1:
                ...
                raise ValueError(f"Error in (speectotexttool): {str(e)}")
...
class SpeechToTextToolTranscribe(PipelineTool):
    default_checkpoint = r"c:\Users\didri\Desktop\LLM-models\Audio-Models\faster-whisper-large-v3-turbo-int8float16"
    description = "Fast tool that transcribes audio into text using faster-whisper. It returns the matched words with timestamps."
    name = "transcriber"
    def __init__(self, device="cuda"):
    ...
    inputs = {
        ...
        "txt_output_path": {
            "type": "string",
            "description": "The text path to write to"
        },
        "video_path": {
            "type": "string",
            "description": "the path to video file"
        }
    ...
    def forward(self, inputs):
        ...
        try:
            ...
            for segment in segments:
                with open(text_path, "a", encoding="utf-8") as t:
                     t.write(f"[{segment.start:.2f}s - {segment.end:.2f}s] {segment.text.strip()}\n")
...
class SpeechToTextTool_viral_agent(PipelineTool):
    default_checkpoint = r"c:\Users\didri\Desktop\LLM-models\Audio-Models\faster-whisper-large-v3-turbo-int8float16"
    description = "Fast tool that transcribes audio into text using faster-whisper. It returns the text in a list"
    name = "transcriber"
    inputs = {
        "audio": {
            "type": "audio",
